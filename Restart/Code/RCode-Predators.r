##### Stream signatures predator abundance analysis #####
	## Last updated 2020-10-07 by J.D. Muehlbauer


##### Prep workspace #####

## Set working directory
setwd('C:/Users/jmuehlbauer/Documents/Projects/Subsidies/StreamSignatureFieldAnalysis/Restart')

## Read in data
raw1 <- read.csv('Data/CSVBugs.csv', header = TRUE, 
	na.strings = c('n/a', ' ', '#N/A', '--'))
	## Data exported from "CSVBugs" tab in "Data/SignatureDatasheet.xlsm".
env1 <- read.csv('Data/AllSites.csv', header = TRUE, 
	na.strings = c('n/a', 'N/A', '#N/A'))
	## Comes from field notes about site characteristics.
islandsXS <- read.csv('Data/IslandsCrossSections.csv', header = TRUE, 
	na.strings = c(' '))
...
offline <- as.logical(system(paste("ping -n 1", 'www.google.com'), 
...
packload(c('MuMIn', 'nlme', 'png', 'plots', 'lme4'))
...
## Remove ground sampling sites without specific locations, counts without taxon IDs
...
bugs2 <- subset(bugs1, select = -Code)
	colnames(bugs2)[which(colnames(bugs2) %in% c('Dist', 'Abund'))] <- 
		c('Distance', 'Abundance')
...
colnames(env2) <- c('Site', 'Bank', 'Region', 'Width', 'Order', 'OrderClass', 
	'Geomorph', 'BankType', 'VegFlood', 'VegUp', 'VegShift', 'Location')
...
mega1 <- mega1[, !colnames(mega1) %in% c('Site.y', 'Bank.y')]
	colnames(mega1)[which(colnames(mega1) %in% c('Site.x', 'Bank.x'))] <- 
		c('Site', 'Bank')
...
mega1$Group <- factor(mega1$Group, levels = c('AeAqEp', 'AeAqPl', 'AeAqTr', 
	'AeAqDi', 'AeAqOd', 'AqEphe', 'AqPlec', 'AqTric', 'AqMisc', 'AeLepi', 
	'HeHemi', 'HeLepi', 'TeMisc', 'PrBeet', 'PrSpHu', 'PrSpWe'))
mega1$Region <- factor(mega1$Region, 
	levels = c('Coweeta', 'Elbe', 'Danube', 'Tagliamento'))
mega1$Geomorph <- factor(mega1$Geomorph, 
	levels = c('Straight', 'Meandering', 'Point bars', 'Braided'))
mega1$BankType <- factor(mega1$BankType, 
	levels = c('Gradual', 'Steep', 'Terrace', 'Levee'))
mega1$VegFlood <- factor(mega1$VegFlood, 
	levels = c('Barren', 'Understory', 'Overstory'))
mega1$VegUp <- factor(mega1$VegUp, levels = c('Understory', 'Overstory'))
mega1$VegShift <- factor(mega1$VegShift, 
	levels = levels(mega1$VegShift)[c(3, 2, 1, 5, 4)])
...
ext1 <- c('CORN1.1RB', 'CORN1.6LB', 'DANU1.2RB', 'ELBE1.2LB', 'FELL1.1RB', 
	'ISLA1.1RB', 'ISLA1.7LB', 'RESI1.1RB', 'RESI1.2LB', 'RESI5.1RB', 'RESI5.2LB')
...
## Get rid of night/malaise/ground sampling
	## These are poorly quantifiable (more a CPUE than a density), 
	## and only 159/3547 samples anyway.
...
## Add sweeps to each pitfall/bowl replicate
	...
	## Thus, any missing sweeps should be interpreted as 0s, not NA 
	## (so they can be added to pitfalls).
	## Distribute entire contents of sweep to every pitfall replicate
	## (don't do proportions).
mega4$Code <- with(mega4, paste0(SiteBank, Distance, Group))
pibo1 <- with(mega4, mega4[Method %in% c('Pitfall', 'Bowl'),])
swee1 <- with(mega4, mega4[Method == 'Sweep' & Abundance > 0 & Code %in% 
	pibo1$Code,])
swee2 <- with(mega4, mega4[Method == 'Sweep' & Abundance > 0 & !Code %in% 
	pibo1$Code,])
...
## Remove NA biomass rows and "2" subset sites (and Mud Creek and Haw River)
mega6 <- mega5[!is.na(mega5$Biomass) & !mega5$Site %in% 
	c('BALL2', 'COWE2', 'LTEN2', 'HAWR1', 'MUDC1'),]
	
## Create Abundance*Biomass column
mega6$AbBiom <- mega6$Abundance * mega6$Biomass

## Re-order columns, remove unnecessary rows
mega7 <- mega6[, c('SiteBank', 'Site', 'Bank', 'Distance', 'Group', 'Trophic', 
	'Abundance', 'Biomass', 'AbBiom', 'Region', 'Width', 'Order', 'OrderClass', 
	'Geomorph', 'BankType', 'VegFlood', 'VegUp', 'VegShift')]
...
plotting('Predator', pred1, 'Distance', 'Abundance', 'SiteBank', filetype = 'pdf')	
plotting('Predator', pred1, 'Distance', 'Biomass', 'SiteBank', filetype = 'pdf')	
plotting('Predator', pred1, 'Distance', 'AbBiom', 'SiteBank', 
	'Abundance x Biomass', filetype = 'pdf')
	## Could also plot by shortSite instead of SiteBank, but don't see need now.
	## Could also plot pngs if needed for manuscript.
### STOPPED HERE.		
